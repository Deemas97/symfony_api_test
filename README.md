# Запуск проекта Symfony REST-приложения

## Порядок установки

1. Переименуйте файл ".env.example" в ".env"
    для подключения приложения к БД контейнера "app_postgres".
> ! Значения переменных БД должны соответствовать значениям из файла /postgres-init/init.sql !

1. Запустить Makefile командой `make init`
> ! Рекомендуется отключить антивирус на время работы с проектом во избежание блокировок соединения с компонентами контейнера !

> ! Не применяйте изменения файла Dockerfile по рекомендациям от Composer !

3. Выполните команду `php bin/console doctrine:migrations:migrate`
    для создания таблиц базы данных.

4. Выполните команду `php bin/console doctrine:fixtures:load`
    для генерации тестовых записей в таблицы базы данных.

## Проверка работы проекта

Для проверки работоспособности запустите тест IntegratedTest.php при помощи инструмента PHPUnit
